...
## Автоматичне оновлення (Claude Scheduled Task) — без сервера й без API-ключа

Замість того щоб тримати живий сервер (VPS/Railway/Render) і платити за
Anthropic API, оновлення `docs/grants.json` робить запланована задача Claude
(підписка користувача, не API-білінг):

1. `scripts/fetch_raw.py` — тягне сирі оголошення з джерел (`SOURCES` у
   `app/pipeline/run.py`) без жодного LLM-виклику, друкує JSON.
2. Claude (у сесії запланованої задачі) читає цей JSON, сам виконує
   структуровану екстракцію/фільтрацію/оцінку за критеріями громади
   (Львівська область, прийнятність ОМС, дедлайн, ймовірність успіху —
   логіка як у `app/pipeline/match.py`/`score.py`), формує
   `docs/grants.json`.
3. Комітить і пушить зміни в `main`.
4. `docs/grants.json` роздається безкоштовно через GitHub Pages —
   сайт громади робить `fetch()` цього URL і рендерить картки на клієнті.

Це не заміняє `app/pipeline/extract.py` (Anthropic API) — той лишається як
альтернативний шлях для локального запуску FastAPI-дашборду (`/api/refresh`),
якщо колись знадобиться живий бекенд. Для продакшн-моніторингу через
scheduled task він не використовується.
